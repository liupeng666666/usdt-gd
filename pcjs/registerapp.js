...
function showRegisterProtocol() {
    $('.serverBoxs').show();
}

function agreeBtn(num) {
    if (num === 1) {
        $('#register_protocol').prop("checked", true)
    } else {
        $('#register_protocol').prop("checked", false)
    }
    $('.serverBoxs').hide();
}
...
function textboxBlur() {
    ...
    $("#password_email").blur(function () {
        ...
        if (password == undefined || password == null || password == "") {
            ...
            //alert("密码至少包含一个大写字母、一个小写字母、一个数字、一个特殊符号");
        } else if (trade_password.length > 0 && trade_password == password) {
            alert("交易密码不能和登陆密码一致");
        }
    });
